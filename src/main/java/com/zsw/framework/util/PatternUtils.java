...
public class PatternUtils {
	 	 ...
	 	 * */
	   public static boolean wildMatchIgnoreCase(String pattern, String str) {
	        pattern = toJavaPattern(pattern);
	        return java.util.regex.Pattern.matches(pattern, str.toLowerCase());
	    }
...
//	        	 pattern = pattern.substring(0, pattern.indexOf("/*"))+"(/+)*";
//	         }
//	    	System.out.println(wildMatch("*insert*|*save*|*add*", "save"));
//	         System.out.println(java.util.regex.Pattern.matches(pattern, str));
//	         test(pattern, str);	
	    	
//	    	   PatternMatcher pathMatcher = new AntPathMatcher();
//		        System.out.println("/**"+pathMatcher.matches("/**","/login/add"));
		        
//	    	 test("*insert*|*save*|*add*","save()");
